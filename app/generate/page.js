    ...
    return (
        ...
        <div className='bg-[#FFFFFF] min-h-screen grid grid-cols-2'>
            <div className="col1">
                ...
                <div className='text-center mt-10'>
                    <div className='flex flex-col justify-center items-center gap-3'>
                        <h1 className='font-bold text-4xl mb-2'>Create your Linktree</h1>
                        <div className="item">
                            <h2 className='font-semibold text-xl'>Step 1: Claim your Handle</h2>
                            <input value={handle || ""} onChange={e => { sethandle(e.target.value) }} className='bg-[#F6F7F5] px-4 py-2 my-2 rounded-full focus:outline-slate-600' type="text" placeholder='Choose a handle' />
                        </div>
                        <div className="item">
                            <h2 className='font-semibold text-xl'>Step 2: Add links</h2>
                            {links && links.map((item, index) => {
                                return <div key={index}>
                                    <input value={item.linktext || ""} onChange={e => { handlechange(index, item.link, e.target.value) }} className='bg-[#F6F7F5] px-4 py-2 my-2 mx-2 rounded-full focus:outline-slate-600' type="text" placeholder='Enter link text' />
                                    <input value={item.link || ""} onChange={e => { handlechange(index, e.target.value, item.linktext) }} className='bg-[#F6F7F5] px-4 py-2 my-2 mx-2 gap-3 rounded-full focus:outline-slate-600' type="text" placeholder='Enter link' />
                                </div>
                            })}
                            <button onClick={() => addLink()} type="button" className="text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-full text-sm px-5 py-3 text-center shadow-md hover:shadow-lg transition-all duration-150 dark:bg-purple-500 dark:hover:bg-purple-600 dark:focus:ring-purple-800 mx-2">+ Add links</button>
                        </div>
                        <div className="item">
                            <h2 className='font-semibold text-xl'>Step 3: Add Picture & Description</h2>
                            <input value={pic || ""} onChange={e => { setpic(e.target.value) }} className='bg-[#F6F7F5] px-4 py-2 my-2 rounded-full focus:outline-slate-600' type="text" placeholder='Enter img link' />
                            <input value={desc || ""} onChange={e => { setdesc(e.target.value) }} className='bg-[#F6F7F5] px-4 py-2 my-2 rounded-full focus:outline-slate-600' type="text" placeholder='Enter description' />
                            <button disabled={pic == "" || handle == "" || links[0].linktext == ""} onClick={() => { submitLinks() }} type="button" className="disabled:bg-purple-400 text-white w-1/2 mt-4 bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-full text-lg px-2 py-2 text-center shadow-md hover:shadow-lg transition-all duration-150 dark:bg-purple-500 dark:hover:bg-purple-600 dark:focus:ring-purple-800">Create Linktree</button>
                        </div>
                    </div>
                </div>
            ...
            <div className="col2 w-full h-screen">
                <img className='w-full h-screen object-cover' src="/generate3.png" alt="Generate your links" />
                <ToastContainer />
            </div>
        </div>
    )
